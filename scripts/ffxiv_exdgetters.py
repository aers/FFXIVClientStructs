...
if api is None:
    try:
        ...
        print("Script has moved to new location inside of ida folder.")
        exit(1)
        ...
        class GhidraApi(BaseApi):
            def create_enum_struct(self, name, values, width=0):
                ...
                enum_dt = EnumDataType(
                    path.getCategoryPath(), path.getDataTypeName(), width or 8
                )
            ...
            def create_struct(self, name, fields):
                ...
                struct = StructureDataType(
                    dt_path.getCategoryPath(), dt_path.getDataTypeName(), 0
                )
                struct.setExplicitMinimumAlignment(4)
                for [offset, [type_name, field_name]] in fields.items():
                    if monitor.isCancelled():
                        break
                    field_dt = currentProgram.getDataTypeManager().getDataType(
                        self.get_datatype_path(type_name)
                    )
                    if field_dt is None:
                        print(
                            f"Warning: Data type {name} field {field_name} has missing type {type_name} ({dt_path})"
                        )
                        continue
            ...
            def process_pattern(self, pattern):
                ...
                pattern = "".join(
                    ["\\x" + x if x != "?" else "." for x in pattern.split(" ")]
                )
            ...
            def do_pattern(self, pattern, suffix):
                ...
                address_set = currentProgram.getAddressFactory().getAddressSet(
                ...
                for ea in result_list:
                    ...
                    if sheet_name is None:
                        print(
                            f"Warning: sheet index {sheet_index} is undefined at {ea}"
                        )
                        continue
                    func_name = f"Component::Exd::ExdModule.Get{sheet_name}{suffix}"
                    self.set_func_name(
                        ea, func_name, f"Sheet: {sheet_name} ({sheet_index})"
                    )
                    ...
                    arg_vars.add(
                        ParameterImpl(
                            "rowId", UnsignedIntegerDataType(), currentProgram
                        )
                    )
                    if suffix == "RowAndSubRowId":
                        arg_vars.add(
                            ParameterImpl(
                                "subRowId", UnsignedIntegerDataType(), currentProgram
                            )
                        )
            ...
            def get_rdx_arg(self, insn, min_address):
                ...
                while (
                    insn is not None and insn.getMinAddress().getOffset() >= min_address
                ):
                    if monitor.isCancelled():
                        break
...
exd_func_patterns: dict[str, tuple[str, tuple[str | None, int]]] = {
    ...
    "48 83 EC 28 48 8B 05 ? ? ? ? BA ? ? ? ? 44 0F B6 C1 48 8B 88 ? ? ? ? E8": (
        "Row",
        ("48 8B 05 ? ? ? ? BA", 8),
    ),
    "48 83 EC 28 48 8B 05 ? ? ? ? BA ? ? ? ? 44 0F B7 C1 48 8B 88 ? ? ? ? E8": (
        "Row",
        ("48 8B 05 ? ? ? ? BA", 8),
    ),
    ...
    "48 83 EC 28 48 8B 05 ? ? ? ? BA ? ? ? ? 48 8B 88 ? ? ? ? E8": (
        "SheetIndex",
        ("48 8B 05 ? ? ? ? BA", 8),
    ),
...
for key in exd_headers:
    ...
    for enum_key in exd_header_enums:
        api.create_enum_struct(
            f"{struct_name}::{enum_key}", exd_header_enums[enum_key], 1
        )
