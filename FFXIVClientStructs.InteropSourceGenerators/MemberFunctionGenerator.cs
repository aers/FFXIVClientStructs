...
internal sealed class MemberFunctionGenerator : IIncrementalGenerator
{
        ...
        ImmutableArray<Validation<DiagnosticInfo, StructWithMemberFunctionInfos>> structInfos)
    {
        ...
        builder.AppendLine("{");
        builder.Indent();
        builder.AppendLine("private void AddMemberFunctions()");
        builder.AppendLine("{");
        builder.Indent();
    ...
    internal sealed record MemberFunctionInfo(MethodInfo MethodInfo, SignatureInfo SignatureInfo)
    {
        ...
        {
            builder.AppendLine($"RegisterSignature(new Signature(\"{structInfo.Name}.{MethodInfo.Name}\", \"{SignatureInfo.Signature}\", {SignatureInfo.GetByteArrayString()}, {SignatureInfo.GetMaskArrayString()}, 0, {GetLambdaAddressSetter(structInfo)}));");
        }

        private string GetLambdaAddressSetter(StructInfo structInfo)
        {
            string hierarchy = structInfo.Hierarchy.Any() ? "." + string.Join(".", structInfo.Hierarchy) : "";
            string thisPtrString = MethodInfo.IsStatic ? "" : "global::" + structInfo.Namespace + hierarchy + "." + structInfo.GetThisPtrTypeString();
            return
                $"(address) => {structInfo.Namespace}.{structInfo.Name}.MemberFunctionPointers.{MethodInfo.Name} = (delegate* unmanaged[Stdcall]<{thisPtrString}{MethodInfo.GetParameterTypeString()}{MethodInfo.ReturnType}>)address";
        }
    }
