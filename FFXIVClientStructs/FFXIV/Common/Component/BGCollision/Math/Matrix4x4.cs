...
public unsafe struct Matrix4x4 : IEquatable<Matrix4x4> {
    [FieldOffset(0x00), CExportIgnore] public fixed float Matrix[16];
    ...
    public Common.Math.Vector3 Translation {
    ...
    public static Matrix4x4 CreateLookAt(Common.Math.Vector3 cameraPosition, Common.Math.Vector3 cameraTarget, Common.Math.Vector3 cameraUpVector) {
        var zaxis = Common.Math.Vector3.Normalize(cameraPosition - cameraTarget);
        var xaxis = Common.Math.Vector3.Normalize(Common.Math.Vector3.Cross(cameraUpVector, zaxis));
        var yaxis = Common.Math.Vector3.Cross(zaxis, xaxis);
        ...
        return Identity with {
            ...
            M23 = zaxis.Y,
            M33 = zaxis.Z,
            M41 = -Common.Math.Vector3.Dot(xaxis, cameraPosition),
            M42 = -Common.Math.Vector3.Dot(yaxis, cameraPosition),
            M43 = -Common.Math.Vector3.Dot(zaxis, cameraPosition)
        };
    }

    public static Matrix4x4 CreateWorld(Common.Math.Vector3 position, Common.Math.Vector3 forward, Common.Math.Vector3 up) {
        var zaxis = Common.Math.Vector3.Normalize(-forward);
        var xaxis = Common.Math.Vector3.Normalize(Common.Math.Vector3.Cross(up, zaxis));
        var yaxis = Common.Math.Vector3.Cross(zaxis, xaxis);
        return Identity with {
            M11 = xaxis.X,
            M12 = xaxis.Y,
    ...
    public static bool Decompose(Matrix4x4 matrix, out Common.Math.Vector3 scale, out Quaternion rotation, out Common.Math.Vector3 translation) {
