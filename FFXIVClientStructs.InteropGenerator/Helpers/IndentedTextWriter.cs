...
internal sealed class IndentedTextWriter : IDisposable
{
    ...
    public Span<char> Advance(int requestedSize)
    {
        ...
        if (this.builder.Count == 0 || this.builder.WrittenSpan[^2..].SequenceEqual(DefaultNewLine.AsSpan()))
        {
    ...
    {
        if (skipIfPresent && this.builder.WrittenSpan is [.., '\r', '\n', '\r', '\n'])
        {
            return;
        }

        this.builder.AddRange(DefaultNewLine.AsSpan());
    }
